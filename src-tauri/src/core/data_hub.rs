...
pub async fn run_data_hub_manager(mut receiver: mpsc::Receiver<HubMessage>, app_handle: AppHandle) {
            ...
            _ = flush_interval.tick() => {
                ...
                if !updates.is_empty() {
                    ...
                    for (id, payload) in updates {
                        tags_map.insert(id, payload);
                    }

                    let event = TagsUpdatedEvent { tags: tags_map };

                    if let Err(e) = app_handle.emit("tags-updated", event) {
                        eprintln!("Failed to emit tags-updated event: {}", e);
                    }
                }
            }
